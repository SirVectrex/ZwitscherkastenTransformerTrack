...
CONFIG = {
    "paths": {
        ...
        "metrics_dir": CURRENT_DIR / "metrics",
...
def main():
    ...
    # Load idx to class mapping for logging
    ...
    json_metrics_path = CONFIG["paths"]["metrics_dir"] / f"{run_name}_metrics.json"
    
    # Initialize CSV with headers
    with open(csv_log_path, 'w', newline='') as f:
        csv.writer(f).writerow([
            'epoch', 'train_loss', 'train_acc', 'train_top5_acc', 
            'val_loss', 'val_acc', 'val_top5_acc', 'lr'
        ])
    
    # Initialize JSON metrics storage
    metrics_history = {
        "config": CONFIG,
        "phase": PHASE,
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(
        ...
        batch_size = CONFIG["hyperparams"]["batch_size"]
        steps_per_epoch=len(train_loader) // (batch_size // CONFIG["hyperparams"]["batch_size"]),
    ...
    for epoch in range(CONFIG["hyperparams"]["epochs"]):
        ...
        for i, (images, labels) in enumerate(pbar):
            ...
            if (i + 1) % accum_steps == 0 or (i + 1) == len(train_loader):
                torch.nn.utils.clip_grad_norm_(
                    model.parameters(), 
                    CONFIG["hyperparams"]["max_grad_norm"]
                )
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad(set_to_none=True)
        ...
        with torch.no_grad():
            pbar_val = tqdm(val_loader, desc=f"Epoch {epoch+1}/{CONFIG['hyperparams']['epochs']} [VAL]")
        ...
        print(f"   Train Loss: {avg_train_loss:.6f} | Train Acc: {train_acc:.2f}% | Train Top-5: {train_top5_acc:.2f}%")
        print(f"   Val Loss:   {avg_val_loss:.6f} | Val Acc:   {val_acc:.2f}% | Val Top-5:   {val_top5_acc:.2f}%")
        ...
        if val_acc > best_acc:
            ...
            print(f"🌟 New best Top-1 Acc: {val_acc:.2f}% | Top-5 Acc: {val_top5_acc:.2f}% (saved)")
