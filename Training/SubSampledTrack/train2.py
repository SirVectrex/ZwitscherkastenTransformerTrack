...
PHASE = 2  
MIXUP_ALPHA = 0 #0.4 if PHASE == 2 else 0.0

CONFIG = {
    "paths": {
        # --- INPUTS (Von der Festplatte) ---
        # Wir erwarten, dass du das Skript aus dem Hauptordner ausführst
        "train_csv": Path("Training/SubSampledTrack/train.csv"),
        "val_csv": Path("Training/SubSampledTrack/val.csv"),
        "stats_json": Path("Training/SubSampledTrack/passt_stats.json"),
        "class_map": Path("Training/SubSampledTrack/class_map.json"),
        
        # --- OUTPUTS (In den RAM / dev/shm) ---
        # Hier wird gespeichert, damit die Disk nicht voll läuft
        "checkpoint_dir": Path("/dev/shm/schoen/checkpoints"),
        "log_dir": Path("/dev/shm/schoen/logs"),
        
        # Lade das beste Modell aus Phase 1 (liegt im RAM)
        "load_checkpoint": Path("/dev/shm/schoen/checkpoints/best_model.pth") 
    },
    "hyperparams": {
        "batch_size": 4, #16, # Klein halten wegen Speicher
        "epochs": 30 if PHASE == 2 else 20,
        "lr": 1e-5 if PHASE == 2 else 1e-3, # Feinere LR für Phase 2
        "weight_decay": 0.0001,
        "label_smoothing": 0, #.1,
        "max_grad_norm": 1.0,
    }
...
def main():
    ...
    if PHASE == 2:
        ...
        if ckpt_path.exists():
            print(f"✅ Loading Phase 1 weights from RAM: {ckpt_path}")
            ...
            try:
                ...
                print(f"⚠️ Partial load warning: {e}")
        else:
            print(f"⚠️ No Phase 1 checkpoint found at {ckpt_path}. Starting fresh.")
    ...
    # 3. DATA LOADERS (Load from Disk)
    print("Loading Datasets from Disk...")
    train_dataset = MelDataset(CONFIG["paths"]["train_csv"], CONFIG["paths"]["stats_json"])
    val_dataset = MelDataset(CONFIG["paths"]["val_csv"], CONFIG["paths"]["stats_json"])
    ...
    print(f"--- STARTING TRAINING (Output -> {CONFIG['paths']['checkpoint_dir']}) ---")
    for epoch in range(CONFIG["hyperparams"]["epochs"]):
        ...
        for i, (images, labels) in enumerate(pbar):
            ...
            # Input Fix: 3D [Batch, 128, 998]
            if images.dim() == 4: images = images.squeeze(1)
            if images.shape[-1] > 998: images = images[:, :, :998]
            ...
            if PHASE == 2 and MIXUP_ALPHA > 0:
                 train_correct += (lam * predicted.eq(labels_a).sum().float() + (1 - lam) * predicted.eq(labels_b).sum().float()).item()
            else:
        ...
        if epoch_val_acc > best_acc:
            ...
            print(f"🌟 Saved new best model to RAM: {save_path}")
